...
from hw3.models.BaseModel import BaseModel
...
class NeuralNetwork(BaseModel):
    ...
    def get_accuracy(self, predictions):
        ...
        print(predictions[0],predictions[3],predictions[5])
        return float(correct)/len(predictions) * 100
    def get_f1(self, predictions):
        r"""
        Method to get f1-score of the learned model on test data
        :param predictions:
        :return: f1 score
        """
        preds = []
        ground_truth = []
        for (y_hat, y) in predictions:
            preds.append(y_hat)
            ground_truth.append(y)
        f1 = metrics.f1_score(ground_truth,preds,labels=[0,1,2],average='macro')
        return f1
